...
const PreLogin = ({ setStep }) => {
	return <div>
		...
		<p className="footer-text">Something is tut in here <button onClick={ () => setStep("register") }>register</button></p>
	</div>;
};
...
const Login = ({ setStep }) => {
	...
	return <div>
		...
		<form onSubmit={ submitForm }>
			...
			<Button className="mr-3" onClick={ () => setStep("forget-password") }>Forget Password</Button>
			<Button type="submit" color="success">Acceed</Button>
			<p className="mt-5">Don't have account <button onClick={ () => setStep("register") }>register</button></p>
...
const Success = () => {
	return <div>
		...
		<p>Lorem ipsum dolor, sit amet consectetur adipisicing elit. Reiciendis suscipit incidunt voluptatum magni nam quam, nostrum id quis tenetur! Ipsam inventore nemo dignissimos consectetur voluptate!</p>
		<Button color="success" size="large">OK</Button>
	</div>;
};
...
const Register = ({ setStep }) => {
	...
	return <div>
		...
		<form onSubmit={ submitForm }>
			...
			<div className="form-group">
				<PhoneInput country={ 'us' } value={ phone } onChange={ p => setPhone(p) } />
			</div>
			<div className="form-group">
...
const MultiStepForm = ({ toggle }) => {
	...
	const RenderComponent = () => {
		...
		if (steps.hasOwnProperty(history.current) && steps[ history.current ] !== undefined) {
